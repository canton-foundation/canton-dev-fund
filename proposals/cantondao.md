...
- **Smart contracts:** Daml — `DAO`, `GovernanceProposal`, `VoteRecord`, `WeightSnapshot` templates live on MainNet and TestNet (`governance-v3` v0.7.1), plus `WalletRegistration` (separate `wallet` package, v0.2.0, both ledgers); `Treasury`, `ProposalDeposit`, `MultisigProposal` in funded scope
...
- Public documentation site at [docs.syncvotes.com](https://docs.syncvotes.com) — user guide (TestNet quickstart, DAO configuration, proposal lifecycle, private DAOs, wallets, institutions), API reference, FAQ, and step-by-step how-tos with screenshots captured live on TestNet
...
- **CIP-0104 attribution correction (v0.6.1, July 14, pkg `abc70a99…`):** the CIP pays confirmers, not observers — app-provider promoted from observer to **signatory** on Public DAO / proposal / vote envelopes; Private stays members-only (privacy > rewards by design); SCU lineage upgrade on both ledgers. The same confirmer rule applied to onboarding: `wallet` package v0.2.0 (pkg `9529bc92…`) has the app-provider co-sign every `WalletRegistration` (additive `Optional` field per SCU rules — all pre-0.2.0 registrations stay valid)
- **Public documentation site:** [docs.syncvotes.com](https://docs.syncvotes.com) — user guide, API reference, FAQ, step-by-step how-tos with live TestNet screenshots
...
The scope of this proposal has expanded materially since the first draft to include the v3 economic engine — per-DAO `Treasury` contracts under CIP-0073 sponsorship, the bounded refund pipeline with fraud detection, deposit-and-slash anti-spam, token-based voting, `MultisigProposal`, SubDAO hierarchies, hardware-wallet support, and a third-party audit by Quantstamp and CredShields. Comparable on-chain governance suites in other ecosystems (DAODAO on Cosmos, Aragon on EVM, OpenZeppelin Governor implementations) are funded at materially higher levels for narrower feature sets and without Canton's privacy requirements. The 700,000 CC total reflects the full v3 deliverable set.

A material share of the total is carrying cost rather than engineering: operating as a Featured App requires a 5,000,000 CC lock under CIP-0116. The author does not hold 5M CC — the lock is rented on the open market, where lock rates currently run 5–6.25% APR, i.e. **250,000–310,000 CC per year of pure carrying cost** before any development work. Together with the third-party audit (Quantstamp + CredShields) and the remaining M2/M3 engineering, the 700,000 CC total is tight rather than generous.

Note that since the May revision, a substantial portion of the originally-funded scope (typed proposals, quorum enforcement, weighted voting, the privacy track, and the CIP-0104 attribution correction) has **already been delivered at the author's expense** — the request total is unchanged.
