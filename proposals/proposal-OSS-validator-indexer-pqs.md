...
This proposal outlines a grant for Digital Asset to: (1) open-source the Participant Query Store (PQS) and transfer its governance to the Canton Foundation[^1] and (2) enhance PQS to be current with Canton Network dApp feature requests and recent Ledger API enhancements.
...
This request is for a grant of 7,800,000 Canton Coin (CC) for turning PQS fully into a public good for the Canton ecosystem and some feature enhancements. It does not request funding for ongoing operational maintenance which would be included in the *2026-Maintenance Grant for Daml Open Source*.
...
1. **Open-Source Transition:** transition to Open-Source licensing and possibly eventual governance by the Canton Foundation in line with the transition of the Daml SDK. The Canton Foundation thereby provides the ecosystem with a SQL querying indexer for private ledger data. Proprietary dependencies are removed and the PQS codebase is released under the Apache 2.0 License. This establishes standardized SQL read access without proprietary lock-in or licensing risk.
...
2. **PQS Enhancements for Canton Network:** PQS requires enhancements for parity with current Canton Network validator features and to fulfill specific dApp integration requirements. These enhancements will be backward compatible. The enhancements are:
...
**M1: Open-Source transition & core decoupling**
...
* **M1 (Open Source):** The PQS and repositories are public under an Open Source license, free of proprietary dependencies, pass 100% of integration tests in a public CI pipeline, and fully prepared to be transitioned to the Canton Foundation GitHub namespace in conjunction with the rest of the Daml SDK.
* **M2 (Enhancements I):**
...
**Milestone 1 (Open-Source Transition):** 3,900,000 CC (50%) upon committee acceptance of deliverables.
