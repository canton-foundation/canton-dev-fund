...
| 2. Core Implementation & Zenith ecosystem integrations - Developer milestone | M4=M2+2 | EVM/Solidity-facing read interface, SDK scaffolding, and feed configuration logic implemented natively for Zenith EVM environments. | Successful execution of the E2E push model utilizing Zenith's EVM external call primitives, evidenced by a documented Mainnet transaction log. | 15% |
| 3. Adoption of Zenith EVM integration - Adoption Milestone | M6=M4+2 | **Production Usage Metric:** Deployment of at least 20 active price feeds on Zenith Mainnet, proven by a verifiable on-chain ledger history of a minimum of 10,000 successful price updates over a continuous 30-day epoch. | **Targeted Use Case Proof:** Active deployment of rates/pricing feeds to support Zenith's enterprise targets, specifically providing verifiable on-chain collateral valuation pricing for assets deployed for one of Paxos Lab or Ctrl+Alt or another key ecosystem partner to facilitate 10,000 transaction events | 15% |
| 4. Complex Data Processing & Bridge Verification - Developer Milestone | M6=M4+2 | Design of a custom data format for cross-chain asset verification. Indexers and connectors developed for bridging external assets to native CIP 56 holdings | Implementation of a tailored data structure serving as the independent attestation layer for cross-chain bridging. | 10% |
| 5. Complex Data Processing & Bridge Verification - Adoption Milestone | M8 = M6 +2 | **Usage Metric:** Verifiable Mainnet attestation of the initial $1M+ LP liquidity bridged from Solana or other networks like Stellar and Base to Canton via Memora's lock-and-mint contracts, relying explicitly on the CAPS framework. | Verifiable transaction logs demonstrating the CAPS framework successfully processing a minimum of 100 distinct bridging events. | 10% |
| 6. Mainnet Rollout with Institutional Grade Price Feeds - Developer Milestone | M10=M8+2 | Architecture guide, operator runbooks, Daml/EVM consumer quickstarts, monitoring dashboards, and final reference implementations (doc packets) are created and delivered. | Active facilitation of Mainnet price data for Temple Digital Group's or other DEXs live trading venues or upcoming physical commodities or other asset class expansion | 10% |
| 7. Institutional Grade Feeds Rollout and Adoption - Adoption Milestone | M12=M10+2 | Architecture guide, operator runbooks, Daml/EVM consumer quickstarts, monitoring dashboards, and final reference implementations (doc packets) are created and delivered. | **Volume Metric:** The infrastructure demonstrably processes price updates corresponding to a threshold of $1M to $2M in facilitated monthly transaction volume across Temple trading pairs or other DEXs trading pairs | 10% |
| 8. Institutional Asset Onboarding & Final Acceptance - Adoption Milestone | M12=M10+2 | Final project review, CIP supporting evidence compilation, and external security audit. Price/Data feed deployment customized for Tier-1 TradFi asset onboarding. | ● Audit remediation cycle (punch-list items) is closed, and all deliverables are formally accepted by the grant committee.<br>● **Concrete Institutional Proof:** Mainnet deployment of pricing/NAV feeds for a major financial institution (e.g., Hamilton Lane, Apollo, or Spiko). Proven via API access logs or joint public announcement of expanding on Canton or a formal Letter of Dependency provided to the committee, or a verifiable Mainnet transaction hash demonstrating an institutional asset event utilizing CAPS. | 5% |
