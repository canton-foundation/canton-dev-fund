...
| Milestone | Timeline (in months) | Deliverable | Acceptance Criteria | Payment Milestone (in % CC of total 15,775,000 CC) |
|---|---|---|---|---|
| 1. Design, Architecture, Technical Specification and Prototyping to PoC level | M2=M0+2 | Approved Daml data model design, formal threat model (covering oracle-operator communication and entitlement escalation), implementation plan, and traffic-cost measurement plan vs. Canton fee parameters. | ● Full protocol specification including the licensing module and fees management module.<br>● Derivation scoped read is successfully demonstrated on DevNet with open source code<br>● Formal CIP drafted and in community review.<br>● PoC contracts code is published in open source github repository | 20% |
| 2. Core Implementation & Zenith EVM Integration | M4=M2+2 | EVM/Solidity-facing read interface, SDK scaffolding, and feed configuration logic are implemented. | ● Successful execution and documented transaction log of the E2E push model for the Zenith subnet (relayers, on-chain contract, and adapters).<br>● Deployment of at least 3 active price feeds on the Zenith subnet with 24/7 monitoring and data collection via both push and pull models. | 30% |
| 3. Complex data processing | M6=M4+2 | Design of a custom data format dedicated to cross-chain assets transport verification. Indexers and connectors developed for cross-chain bridging DevNet deployment are live, initial feeds are registered, and end-to-end flow works. | ● Implementation of tailored data structure capable of transporting payload enabling cross-chain bridging verification<br>● Demonstration of usage of newly created package for the verification of asset bridging process<br>● Deploying Proof of Reserves (PoR) for supporting Bridge Integration for Memora/Obsidian or another major bridging provider | 20% |
| 4. Mainnet Rollout with Institutional Grade Price Feeds | M12=M10+2 | Architecture guide, operator runbooks, Daml/EVM consumer quickstarts, monitoring dashboards, and final reference implementations (doc packets) are created and delivered. | ● Deployment of a set of financial data (fixed income, private credit or structured pricing)<br>● Onboarding of at least two additional partners that will bring institutional data to the ecosystem<br>● Rolling out tier 1 data and price feeds and facilitate execution based on data for any of Temple, Avro, Hecto or another major application on Canton | 20% |
| 5. Documentation, Tooling and Final Acceptance | M24=M12+12 | Final project review, CIP supporting evidence compilation, and external security audit. | ● Audit remediation cycle (punch-list items) is closed, and all deliverables and CIP materials are formally accepted by the grant committee.<br>● Publication of integration docs and guidance for tooling usage<br>● Published case study of first integrations<br>● Price/Data feed deployment for assets onboarding from any of: Hamilton Lane, Apollo, WisdomTree or another major financial institution entering Canton ecosystem | 10% |
