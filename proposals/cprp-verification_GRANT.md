...
This grant delivers the verification infrastructure defined in CIP-YYYY (Party Identity Verification): the system that answers the Working Group's central question — "How do we remove the `.unverified` prefix?" It introduces a four-tier trust model (T1–T4), DNS verification (Phase 1 via featured resolver quorum, Phase 2 via SV consensus), vLEI verification flows, cross-chain identity linking, and a Featured Resolver Registry governed by SV vote. Encrypted fields for confidential metadata are specified but deferred to a follow-up CIP to keep scope tight.

The design phase (Milestone B1) has already been delivered to the Canton Foundation as PR #171 and is being contributed to the ecosystem at no cost. This grant requests funding only for the implementation (B2) and adoption (B3) phases, leveraging Freename's existing verification and trust-evaluation infrastructure and internal team to deliver the reference implementation at substantially reduced cost relative to a greenfield effort.

This is the second of two grants implementing the Canton Party Resolution Protocol (CPRP). The companion grant (Party Name Resolution, ~844,156 CC) delivers the resolver infrastructure that this grant extends with trust evaluation and verification.
...
DNS Verification Flow: entity enables DNSSEC + publishes TXT record (`_canton.<domain> TXT "party=<party_id>"`). Phase 1 (initial deployment): a quorum of featured resolvers verify the DNSSEC chain independently, each publishing a T3 credential. Phase 2 (future Splice enhancement): SV nodes verify directly and the DSO publishes a T1 `CnsDnsClaim` credential. Re-verification every 7 days.
...
Post-Quantum Encrypted Fields (deferred to follow-up CIP): the cryptographic design (ML-KEM-768 + AES-256-GCM + HKDF-SHA256) is documented in the companion specification for future implementation. The core CPRP deployment focuses on names, trust tiers, and DNS/vLEI verification.
...
Featured Resolver Registry: featured resolver status encoded as a standard CN Credential (publisher = DSO, subject = resolver operator), created by SV governance vote, granting T3 issuer status. Annual renewal required. The Tech & Ops Committee can also designate featured resolvers.
...
### Milestone B1: CIP & Verification Architecture (Completed — Donated)
...
- Funding: 0 CC — contributed to the Canton ecosystem at no cost (approximately $40,000 in design and specification work)
- Delivered artifacts:
  ...
  - Encrypted field cryptographic specification (ML-KEM-768 + AES-256-GCM + HKDF-SHA256) — documented for future CIP
  - Credential-based encoding for `ResolverFeaturedStatus` and `CollisionArbitration` (draft)
  - Working Group presentation (January–February 2026) and feedback incorporation across 23 review comments from Digital Asset (Simon Meier)
  - Exit criterion: CIP-YYYY advancement to "Proposed" status by the Working Group (pending)

### Milestone B2: Verification Implementation

- Estimated Delivery: 14 weeks from grant start; B2 work begins after companion grant's A2 delivers the resolver infrastructure on which the trust evaluator depends
...
- Deliverables / Value Metrics:
  ...
  - DNS verification flow (Phase 1: featured resolver quorum, `CnsDnsClaim` credential lifecycle)
  ...
  - Featured resolver status and collision arbitration credentials deployed to TestNet
  - Network discriminator enforcement (cross-network rejection rules)
  - Explorer demo — showing verified names, trust paths, and profiles
  - Privacy review — query privacy audit, threat model documentation
  - Exit criterion: E2E demo where an app resolves a name to a verified party (T3 DNS + T2 vLEI) and executes a Canton transaction using the resolved identity; explorer displays verified name + profile
...
- Deliverables / Value Metrics:
  - Verification extensions to all 3 SDKs (TypeScript, Python, Java): trust evaluator API, credential verifier API
  - Reference custody app demonstrating resolution + verification between counterparties
  ...
  - Verification-specific integration guide (trust policies, vLEI setup)
...
- Deliverables completed as specified for each funded milestone (B2, B3)
- End-to-end demo: name → verified party → Canton transaction using resolved identity (B2)
- Explorer displaying verified names with trust badges for demo parties (B2)
...
Milestone B1 deliverables have already been submitted (PR #171) and are under WG review. Acceptance of CIP-YYYY to "Proposed" status is treated as a precondition to B2 work commencing, not as a payable milestone of this grant.

## Funding

Total Funding Request: 649,351 CC (equivalent to ~100,000 USD at today's rate of 1 CC = $0.1540)

### Funding Rationale

The original CIP-YYYY proposal targeted ~$200,000 in grant funding. The revised request of ~$100,000 reflects three concrete reductions:

- Milestone B1 (design phase, approximately $40,000 in work) has already been completed and delivered as PR #171. Freename is donating this work to the Canton ecosystem at no cost, regardless of grant outcome.
- Milestone B2 (implementation) benefits from substantial reuse of Freename's existing verification and trust-evaluation infrastructure — credential validation pipelines, multi-issuer trust composition, third-party identity API integrations — adapted for Canton's tier model rather than built from scratch.
...
The remaining funding covers the actual implementation cost of B2 and B3 with a modest operating margin sufficient to absorb scope changes identified during Working Group iteration.

### Payment Breakdown by Milestone

- Milestone B1 (CIP & Verification Architecture): 0 CC — donated, delivered as PR #171
- Milestone B2 (Verification Implementation): 422,078 CC upon committee acceptance (~$65,000)
- Milestone B3 (Verification SDK & Ecosystem Adoption): 227,273 CC upon final release and acceptance (~$35,000)

### Volatility Stipulation

The funded portion of the project (B2 + B3) covers approximately 22 weeks (~5 months) of work, with B2 beginning after the companion grant's A2 milestone delivers the resolver infrastructure. The grant is denominated in fixed Canton Coin and will require a re-evaluation at the 6-month mark per CIP-0100 procedures.
...
Why post-quantum encryption (deferred): Canton's institutional users hold assets with multi-decade time horizons (30-year Treasury securities, long-dated derivatives). ML-KEM-768 provides NIST-standardized post-quantum security for encrypted metadata that may remain sensitive for the full asset lifecycle. The full cryptographic design is documented in the companion specification; implementation is deferred to a follow-up CIP to keep B2 scope tight.

Why separate from resolution: resolution and verification are independently useful and independently evolvable. An explorer adopts resolution first and adds verification later. A compliance tool verifies without resolving (receives a Party ID directly). Coupling them would force unnecessary dependencies.

Why Freename: ICANN-accredited registrar with patented collision resolution technology across identity registries, multi-chain naming infrastructure, and operational experience with cross-registry dispute arbitration — all directly applicable to Canton's multi-resolver trust model. The verification stack adapts existing Freename components (credential validation, third-party identity integrations, multi-issuer trust composition), which is precisely what makes the reduced grant ask feasible.
