...
The journal is the reproduction substrate for the testing component. Each entry records the normalized command or script input, acting and reading parties, package and script/DAR identifiers, named target, participant and synchronizer identity, and the resulting response or rejection. `dpm runlog rerun` reconstructs a supported `dpm exercise` request or table-row script invocation, generates fresh command and submission identifiers to avoid Ledger API deduplication, submits it to the selected compatible target, and records the attempt as a new run linked to the source run. Target identity mismatches are refused, and a rerun against a target marked as production requires `--allow-production`.
...
**Diagnostics interoperability.** A successful journal entry carries its target, party projection, and update ID so DPM Trace (PR #327) can inspect the committed update without resubmitting the command, subject to ledger retention and visibility. A rejected entry instead carries the sanitized submission context and raw, undecoded Ledger API rejection with its correlation ID so the failure-classification engine (PR #297) can diagnose the failure without attempting the submission again. Visual tooling such as DPM Debug (PR #494) can consume the same published journal and trace schemas.
...
**Table-driven and recorded testing.** Generative testing for Daml is being addressed by complementary ecosystem work: OpenZeppelin's experimental [daml-props](https://github.com/OpenZeppelin/daml-props) library provides generators, property checks, state-sequence testing, and shrinking inside Daml, while the proposed [DamlFuzz](https://github.com/canton-foundation/canton-dev-fund/pull/52) project targets property-based and coverage-guided fuzzing. This suite does not implement generators, shrinking, coverage analysis, or a fuzzing engine. Instead, it provides a deterministic command-line runner for developer-selected regression cases and structured CI reporting.

`dpm test-table <module>:<script> --rows rows.json [--target T] [--json]` executes the same Daml Script once for each predefined input row, using `dpm script --input-file` as the underlying execution mechanism. Each result includes the row name, input hash, target, exit status, script output, and available error information. Rows run sequentially unless explicitly configured otherwise, and state changes made by one row may affect later rows unless the target is reset between executions.

Each table row is recorded in the run journal, allowing its input and execution context to be inspected and rerun through `dpm runlog rerun`. This provides repeatable re-execution, not a guarantee of an identical outcome: equivalent results depend on compatible packages, parties, permissions, contract state, and ledger time.

External property-based or fuzzing tools may publish results into the journal's versioned JSON schema. Where those tools expose a reproducibility seed or minimized counterexample, the journal records it alongside the script input. The suite records the launched script's inputs, outputs, and status; it does not claim to intercept every internal submission made by an unmodified Daml Script, and it requires no compiler or Canton runtime changes.
...
* Table-driven testing with journal-backed reruns of recorded inputs, plus a published journal schema that ecosystem testing engines (daml-props, DamlFuzz) can build on
...
*Goal: every suite-driven run is recorded with its LocalNet, DevNet, TestNet, or MainNet target identity and its supported inputs can be reconstructed and rerun against a compatible selected target; table-driven testing lands as the suite's testing capability.*
- Run journal: automatic capture of suite-driven submissions (target/network identity, inputs, events, visibility, abort/assert messages); `dpm runlog list / show / rerun`; published, versioned line-delimited JSON schema; successful entries carrying the update ID and rejected entries carrying the unmodified rejection payload for hand-off to the PR #327 / #297 tools.
- `dpm runlog rerun <run-id>`: reconstruction of supported exercise submissions and table-row invocations, fresh command/submission identifiers, linked journal entries for each rerun, target-identity compatibility checks, and an explicit production-target safeguard; documentation states that identical outcomes are not guaranteed.
- `dpm test-table <module>:<script> --rows rows.json [--target T] [--json]`: deterministic parameterized execution from rows files and per-row structured reporting, with each row recorded for later inspection or rerun.
- Documented integration with a compatible environment-level snapshot/reset component where available, such as the Ledger Snapshot component proposed in PR #604, for repeatable campaign state.
...
- **M4:** a deliberately failing exercise produces a schema-valid journal entry containing the selected target/network identity, submitted inputs, visibility, decoded message, and unmodified rejection payload consumable by PR #297; a successful exercise records its update ID for PR #327. A table campaign containing a deliberately failing row reports the failure per row; `dpm runlog rerun` reconstructs that row with fresh identifiers, records the new attempt as linked to the original, refuses a mismatched target, and documents that the result may differ when ledger state or authorization has changed.
