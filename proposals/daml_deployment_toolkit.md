...
**Daml Deployment Plugin** will close that gap. It is a DPM component — published to an OCI registry and installed via dpm install package — that registers a dpm canton-deploy subcommand and handles the full Daml deployment lifecycle to any Canton validator (LocalNet, TestNet, or MainNet) from one config file with named per-network profiles. It supports both the Admin API and Ledger API upload paths, is multi-synchronizer, integrates cleanly with DPM-built packages, and reads Canton protobufs from the active SDK install.
...
### Milestone 2: _(Launch CLI tool with Documentation, OCI Distribution)_
...
- OCI component release (Milestone 2) and docs published.
...
- Milestone 2 _(Launch CLI tool with Documentation, OCI Distribution)_: 50,000 CC upon committee acceptance
