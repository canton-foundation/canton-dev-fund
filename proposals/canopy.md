...
- Investor whitelisting via Canton's native credential system
...
| `FundShare` | CIP-56 share token. Each fund gets a unique token with its own metadata (fund name, manager, basket info). Transfer restrictions, investor permissioning, balance tracking. Fungible within a fund, distinct across funds. |
...
- **Investor permissioning**: Subscription and redemption choices validate the exercising party against an on-ledger investor whitelist. Unauthorized parties cannot subscribe, redeem, or view fund state.
...
- **Custodial services**: Canopy does not custody assets on behalf of investors. Fund managers and investors manage their own keys.
- **Investment advice or compliance opinions**: The protocol enforces configurable rules (lockups, whitelisting, fees) but does not provide legal, regulatory, or investment guidance.
