...
**Validation and adoption path:** The schema will be published with a minimum 2-week community feedback period (Canton developer forum + Discord) before milestone claim. The team will actively solicit review from at least 2 Canton application builders and 1 compliance-adjacent stakeholder during the feedback period. The schema is published as a versioned open standard under MIT license — not a unilateral specification. If the community or Canton Foundation initiates a formal CIP process for audit package standards, this schema is offered as the starting point, not the final word. Fragmentation is mitigated by publishing first and openly: a concrete, working schema with reference implementation is harder to ignore than a competing spec with no tooling behind it.

**Alignment with existing standards.** No Canton-specific compliance export standard exists today; this schema would be the first. The design references and aligns with: ISO 24165 (Digital Token Identifier) for tokenized asset identification, now required under EU MiCA; ISO 17442 (LEI/vLEI) for optional legal entity identification in the party directory; the DTCC/Clearstream/Euroclear Digital Asset Securities Control Principles (DASCP) for record-keeping and audit trail requirements; and ESMA's DLT Pilot Regime JSON Schema precedent for regulatory-grade DLT reporting. The schema fields are designed to be mappable to FINOS Common Domain Model (CDM) event structures for participants integrating Canton activity into CDM-based post-trade workflows. As ISO/CD TS 23353 (DLT Auditing Guidelines) moves toward publication, the schema will incorporate its audit data requirements in a subsequent version.
...
**Framework independence.** The decoder and connector are pure TypeScript/JavaScript libraries with no React, Next.js, or frontend framework dependency. They run in Node.js, Deno, Bun, or any browser environment. A team building a Vue, Angular, or server-side application consumes the same `@canton-workbench/pqs-decoder` package — the Next.js reference frontend is one integration, not the only integration. The connector exposes a typed query interface that any backend can call. The decoder accepts a JSON payload and returns a typed object — it has no opinion about how that object is rendered. The compliance export schema is a JSON Schema definition, consumable by any language or framework. The Next.js reference frontend demonstrates one complete integration path. The libraries are designed so that alternative paths require no modification to the core packages.
...
- `@canton-workbench/pqs-decoder` + `@canton-workbench/pqs-connector`: ~30% — net-new production libraries extracted from the demo's architecture, with full test coverage, npm packaging, and documentation
- Reference frontend: ~25% — the demo UI exists but uses mock data; the funded work is the live PQS integration, MainNet testing, multi-party verification, and deployment hardening
- Compliance export schema: ~15% — schema definition, JSON Schema validation, PDF conformance, community feedback process
- 90-day maintenance window (M4): ~20% — ongoing SDK compatibility, bug fixes, community support, and runbook delivery
- Documentation, testing, and DevOps: ~10% — LocalNet environment, deployment guides, integration guides, Canton docs PR

The existing demo validates the UX approach and component architecture using mock data. The grant funds the production engineering: typed library extraction with full test coverage, live PQS integration against TestNet/DevNet and MainNet, compliance schema standardization, deployment hardening, and sustained maintenance.
...
**Post-launch adoption targets (12 months):**

- 3+ documented deployments (institutional or node operator), anonymized where required
- 500+ weekly npm downloads for the decoder library (indicating active developer usage)
- At least 2 third-party integrations or forks extending the decoder for custom template types
- Compliance export schema referenced or adopted by at least 1 application builder outside the core team
