...
Canton Network is the venue for Capital Markets at large to move to on-chain rails. The Canton Foundation Development Fund’s priorities are chosen to support this vision, focussing on prerequisites for scalability, resiliency, ease of use, and capability. The below paints a picture of where the Canton Network is headed.
...
Canton Network is able to support this scale through its Network of Networks architecture where all on-chain volume, no matter where it happens - on the Global Synchronizer or use-case-specific dedicated synchronizers -  is part of the same fully interoperable MainNet. The utility generated by this technology and network is tied back into network tokenomics, and attributes value back to those that run and build the network.
...
- Validator operators can self-onboard onboard to the network, merely requiring a traffic purchase to get started.
...
- An active market of providers offers credentials for onchain identity and qualification, based on a common standard for party metadata. A global asset registry builds on this common metadata standard.
- Developers can choose their path to build applications
